...
class SegEvaluator(Evaluator):
    ...
    def sliding_eval(self, img, label, name, crop_size, stride_rate, device=None):
        ...
        processed_pred = self.scale_process(img, label, name,
                                            (ori_rows, ori_cols),
    ...
    def scale_process(self, img, label, name, ori_shape, crop_size, stride_rate,
        ...
        if long_size <= crop_size:
            ...
            label, _ = pad_image_to_shape(label, crop_size,
                                          cv2.BORDER_CONSTANT, value=0)
            score = self.val_func_process(input_data, label, name, device)
            ...
            for grid_yidx in range(r_grid):
                for grid_xidx in range(c_grid):
                    ...
                    label_sub, _ = pad_image_to_shape(label_sub, crop_size,
                                                      cv2.BORDER_CONSTANT,
                                                      value=0)
                    temp_score = self.val_func_process(input_data, label_sub,
                                                       name, device)
    ...
    def val_func_process(self, input_data, label, name, device=None):
        ...
        with torch.cuda.device(input_data.get_device()):
            ...
            with torch.no_grad():
                score = self.val_func(input_data, name)
