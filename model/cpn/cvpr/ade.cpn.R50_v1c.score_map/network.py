...
class CPNet(nn.Module):
    def __init__(self, out_planes, criterion, pretrained_model=None,
        ...
        self.context = ObjectContext(2048, out_planes, norm_layer)

        self.head_layer = nn.Sequential(
            ConvBnRelu(3 * out_planes, out_planes, 3, 1, 1,
                       has_bn=True,
    ...
    def forward(self, data, label=None, aux_label=None):
        ...
        if label is not None:
            ...
            antimagnet_loss = self.antimagnet_criterion(intra_sim_map,
                                                        aux_label)
            loss = main_loss + 0.4 * aux_loss + aux_loss2 + intra_sim_loss + antimagnet_loss
            return loss
...
class ObjectContext(nn.Module):
    def __init__(self, in_channels, out_planes, norm_layer=nn.BatchNorm2d):
        ...
        self.local_aggregation = nn.Sequential(
            ConvBnRelu(self.in_channels, self.out_planes,
                       3, 1, 1,
    ...
    def forward(self, x):
        ...
        softmax_x = F.softmax(local_x, dim=1)
        ...
        intra_context = intra_context.view(b, self.out_planes, *x.size()[2:])
        ...
        inter_context = inter_context.view(b, self.out_planes, *x.size()[2:])
        inter_context = self.inter_post_conv(inter_context)

        output = torch.cat([local_x, intra_context, inter_context], dim=1)
