...
class CPNet(nn.Module):
    ...
    def forward(self, data, label=None, aux_label=None):
        ...
        fm = self.context(blocks[-1], aux_label)
...
class ObjectContext(nn.Module):
    ...
    def forward(self, x, sim_map):
        ...
        # intra_mask_count = intra_similarity_map.sum(dim=-1, keepdim=True)
        ...
        inter_mask_count = inter_similarity_map.sum(dim=-1, keepdim=True)
        ...
        output = torch.cat([x, inter_context], dim=1)
        return output
