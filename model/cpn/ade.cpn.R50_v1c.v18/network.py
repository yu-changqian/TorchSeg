...
class CPNet(nn.Module):
    ...
    def forward(self, data, label=None, aux_label=None):
        ...
        fm, intra_sim_map, intra_mask = self.context(blocks[-1])
        ...
        if label is not None:
            ...
            cos_loss = self.cos_loss(intra_mask, aux_label)
            print(main_loss, aux_loss, intra_sim_loss, cos_loss)
...
class ObjectContext(nn.Module):
    ...
    def forward(self, x):
        ...
        intra_mask = torch.gt(intra_similarity_map, 0.6).float()
        ...
        inter_mask = torch.gt(inter_similarity_map, 0.6).float()
        ...
        return output, intra_similarity_map, intra_mask
...
class CosineLoss(nn.Module):
    ...
    def forward(self, pred, label):
        cos_distance = 1 - self.cos(pred, label)
