...
class CPNet(nn.Module):
    ...
    def forward(self, data, label=None, aux_label=None):
        ...
        fm, attention_fm, intra_sim_map = self.context(blocks[-1])
...
class ObjectContext(nn.Module):
    def __init__(self, in_channels, inner_channel, norm_layer=nn.BatchNorm2d):
        ...
        self.intra_similarity_branch = nn.Sequential(
            ...
            ConvBnRelu(self.inner_channel, 150, 1, 1, 0,
    ...
    def forward(self, x):
        ...
        attention_feature = self.intra_similarity_branch(value)
        attention_map = attention_feature.view(b, 150, -1)
        tran_attention_map = attention_map.permute(0, 2, 1)
        intra_similarity_map = torch.matmul(tran_attention_map, attention_map)
        intra_similarity_map = (150 ** -.5) * intra_similarity_map
        ...
        return output, attention_feature, intra_similarity_map
