...
class CPNet(nn.Module):
    def __init__(self, out_planes, criterion, pretrained_model=None,
        ...
        self.antimagnet_criterion = AntimagnetLossv6(reduction='mean')
...
class SymmetricConv(nn.Module):
    def __init__(self, in_channels, ksize, norm_layer=nn.BatchNorm2d):
        ...
        self.t1 = nn.Conv2d(in_channels, in_channels, groups=in_channels,
                            kernel_size=(ksize, 1), stride=1, padding=(padding, 0))
        self.t2 = nn.Conv2d(in_channels, in_channels, groups=in_channels,
                            kernel_size=(1, ksize), stride=1, padding=(0, padding))
...
class ObjectContext(nn.Module):
    def __init__(self, in_channels, inner_channel, norm_layer=nn.BatchNorm2d):
        ...
        self.num_pixel = (config.target_size // 8) ** 2
    ...
    def forward(self, x):
        ...
        #
        osize = config.image_height // 8
        tsize = config.prior_size
        # p_value = F.interpolate(value, size=(tsize, tsize), mode='bilinear',
        #                         align_corners=True)

        intra_similarity_map = self.intra_similarity_branch(value)
        intra_similarity_map = intra_similarity_map.view(b, tsize ** 2, -1)
        intra_similarity_map = intra_similarity_map.permute(0, 2, 1)
        intra_similarity_map = torch.sigmoid(intra_similarity_map)

        # intra_similarity_map = F.interpolate(
        #     intra_similarity_map.view(b, 1, tsize ** 2, tsize ** 2), size=(
        #         osize ** 2, osize ** 2),
        #     mode='bilinear',
        #     align_corners=True)
        # intra_similarity_map = intra_similarity_map.squeeze_()

        value = value.view(b, self.inner_channel, -1)
        value = value.permute(0, 2, 1)

        intra_context = torch.bmm(intra_similarity_map, value)
        intra_context = intra_context.div_(self.num_pixel)
        ...
        inter_context = inter_context.div_(self.num_pixel)
