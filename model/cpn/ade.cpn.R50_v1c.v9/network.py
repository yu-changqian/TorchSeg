...
class CPNet(nn.Module):
    def __init__(self, out_planes, criterion, pretrained_model=None,
        ...
        self.psp_layer = PyramidPooling('psp', 2048, norm_layer=norm_layer)
        self.context = ObjectContext(4096, 512, norm_layer)

        self.head_layer = nn.Sequential(
            ConvBnRelu(4096 + 1024, 512, 3, 1, 1,
                       has_bn=True,
        ...
        self.psp_aux_layer = nn.Sequential(
            ConvBnRelu(4096, 512, 3, 1, 1,
                       has_bn=True,
                       has_relu=True, has_bias=False, norm_layer=norm_layer),
            nn.Dropout2d(0.1, inplace=False),
            nn.Conv2d(512, out_planes, kernel_size=1)
        ...
        self.business_layer.append(self.psp_aux_layer)
    ...
    def forward(self, data, label=None, aux_label=None):
        ...
        fm, intra_sim_map = self.context(psp_fm)
        ...
        psp_fm = self.psp_aux_layer(psp_fm)
        ...
        if label is not None:
            ...
            loss = main_loss + 0.4 * (aux_loss + psp_loss) + intra_sim_loss
            return loss
...
class PyramidPooling(nn.Module):
    def __init__(self, name, fc_dim=4096, pool_scales=[1, 2, 3, 6],
                 norm_layer=nn.BatchNorm2d):
