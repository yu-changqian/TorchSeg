...
from seg_opr.loss_opr import PriorLoss
...
class CPNet(nn.Module):
    def __init__(self, out_planes, criterion, pretrained_model=None,
        ...
        self.aux_criterion = PriorLoss(0.125, config.num_classes, 0)
    ...
    def forward(self, data, label=None):
        ...
        if label is not None:
            main_loss = self.criterion(fm, label - 1)
            aux_loss = self.criterion(aux_fm, label - 1)
            prior_loss = self.aux_criterion(intra_sim_map, label)

            loss = main_loss + 0.4 * aux_loss + prior_loss
            return loss
...
class ObjectContext(nn.Module):
    def __init__(self, in_channels, inner_channel, norm_layer=nn.BatchNorm2d):
        ...
        self.intra_similarity_branch = nn.Sequential(
            ...
            nn.Conv2d(self.inner_channel, 4096, 1, 1, 0, groups=16, bias=False),
            norm_layer(4096)
        )
    ...
    def forward(self, x):
        ...
        intra_context = intra_context.div(4096)
        ...
        inter_context = inter_context.div(4096)
