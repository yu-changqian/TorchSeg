...
class CPNet(nn.Module):
    ...
    def forward(self, data, label=None, aux_label=None):
        ...
        # for i in range(intra_sim_map.size(0)):
        #     attention_map = intra_sim_map[i]
        #     attention_map = np.round(attention_map.cpu().numpy() * 255)
        #
        #     sns.heatmap(attention_map, cbar=False, cmap="YlGnBu", xticklabels=False,
        #                 yticklabels=False)
        #     path =  os.path.join('./log/map', name+'.png')
        #     plt.savefig(path, bbox_inches='tight', pad_inches=0, dpi=400)
        #
        #     label = aux_label[i]
        #     label = np.round(label.cpu().numpy() * 255)
        #
        #     sns.heatmap(label, cbar=False, cmap="YlGnBu",
        #                 xticklabels=False,
        #                 yticklabels=False)
        #     path = os.path.join('./log/gt', name + '.png')
        #     plt.savefig(path, bbox_inches='tight', pad_inches=0, dpi=400)
...
class SymmetricConv(nn.Module):
    def __init__(self, in_channels, ksize, norm_layer=nn.BatchNorm2d):
        ...
        self.t1 = nn.Conv2d(in_channels, in_channels, kernel_size=(ksize, 1),
                            groups=in_channels, stride=1, padding=(padding, 0))
        self.t2 = nn.Conv2d(in_channels, in_channels, kernel_size=(1, ksize),
                            groups=in_channels, stride=1, padding=(0, padding))
