...
from seg_opr.loss_opr import MaskBCELoss, AntimagnetLoss
...
class CPNet(nn.Module):
    def __init__(self, out_planes, criterion, pretrained_model=None,
        ...
        mask = 1 - torch.eye(3600)
        self.bce_criterion = MaskBCELoss(mask, reduction='mean')
        self.antimagnet_criterion = AntimagnetLoss(reduction='mean')

    def forward(self, data, label=None, aux_label=None):
        blocks = self.backbone(data)

        fm, intra_sim_map, intra_sim_fm = self.context(blocks[-1])
        ...
        if label is not None:
            ...
            antimagnet_loss = self.antimagnet_criterion(intra_sim_fm,
                                                        aux_label)
...
class ObjectContext(nn.Module):
    ...
    def forward(self, x):
        ...
        intra_similarity_fm = intra_similarity_map
        intra_similarity_map = torch.sigmoid(intra_similarity_map)
        diagonal_matrix = torch.eye(3600, dtype=intra_similarity_map.dtype,
                                    device=intra_similarity_map.device)
        intra_similarity_map = torch.clamp(
            intra_similarity_map + diagonal_matrix, min=0, max=1)
        ...
        return output, intra_similarity_map, intra_similarity_fm
