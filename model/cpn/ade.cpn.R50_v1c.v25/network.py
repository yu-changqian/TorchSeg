...
class AtrousPyramidPooling(nn.Module):
    def __init__(self, in_channel, inner_channel, dilations=[12, 24, 36],
                 norm_layer=nn.BatchNorm2d):
        super(AtrousPyramidPooling, self).__init__()

        self.map_convs = nn.ModuleList([
            nn.Conv2d(in_channel, inner_channel, 1, bias=False),
            nn.Conv2d(in_channel, inner_channel, 3, bias=False,
...
class ObjectContext(nn.Module):
    def __init__(self, in_channels, inner_channel, norm_layer=nn.BatchNorm2d):
        ...
        self.intra_similarity_branch = nn.Sequential(
            AtrousPyramidPooling(self.inner_channel, 256, norm_layer=norm_layer),
            ConvBnRelu(256 * 4, 3600, 1, 1, 0,
