...
class CPNet(nn.Module):
    def __init__(self, out_planes, criterion, pretrained_model=None,
        ...
        self.business_layer.append(self.attention_layer)
    ...
    def forward(self, data, label=None, aux_label=None):
        ...
        fm, attention_map, intra_sim_map = self.context(blocks[-1])
        ...
        attention_fm = self.attention_layer(attention_map)
        attention_fm = F.interpolate(attention_fm, scale_factor=8,
                                     mode='bilinear',
                                     align_corners=True)
...
class ObjectContext(nn.Module):
    def __init__(self, in_channels, inner_channel, norm_layer=nn.BatchNorm2d):
        ...
        self.attention_conv = SymmetricConv(self.inner_channel, 7, norm_layer)
        self.attention_branch = ConvBnRelu(self.inner_channel, 3600, 1, 1, 0,
                                           has_bn=True, has_relu=False,
                                           has_bias=False,
    ...
    def forward(self, x):
        ...
        attention_map = self.attention_conv(value)
        intra_similarity_map = self.attention_branch(attention_map)
        ...
        return output, attention_map, intra_similarity_map
