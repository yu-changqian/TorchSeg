...
class CPNet(nn.Module):
    ...
    def forward(self, data, label=None, aux_label=None):
        ...
        fm, intra_context, intra_sim_map = self.context(blocks[-1])

        # intra_mask = intra_sim_map.squeeze()
        # intra_mask = (intra_mask + intra_mask.t()) / 2
        # intra_mask = (intra_sim_map > 0.5).float()
        # import matplotlib.pyplot as plt
        # plt.figure('show', figsize=(12, 4))
        # plt.subplot(1, 3, 1)
        # plt.imshow(intra_sim_map.squeeze().cpu().numpy())
        #
        # plt.subplot(1, 3, 2)
        # plt.imshow((aux_label-intra_sim_map).squeeze().cpu().numpy())
        #
        # plt.subplot(1, 3, 3)
        # plt.imshow(aux_label.squeeze().cpu().numpy())
        # plt.show()
        ...
        intra_context = F.interpolate(intra_context, scale_factor=8,
                                      mode='bilinear',
                                      align_corners=True)
        ...
        if label is not None:
            ...
            intra_context_loss = self.criterion(intra_context, label)
            intra_sim_loss = self.bce_criterion(intra_sim_map, aux_label)
            antimagnet_loss = self.antimagnet_criterion(intra_sim_map,
                                                        aux_label)
            loss = main_loss + intra_context_loss + 0.4 * aux_loss + intra_sim_loss + antimagnet_loss
            return loss
...
class ObjectContext(nn.Module):
    ...
    def forward(self, x):
        ...
        # for i in range(intra_similarity_map.size(0)):
        #     intra_similarity_map[i, :, :] += intra_similarity_map[i, :, :].t()
        # intra_similarity_map /= 2
        ...
        return output, intra_context, intra_similarity_map
