...
class CPNet(nn.Module):
    ...
    def forward(self, data, label=None, aux_label=None):
        ...
        if label is not None:
            ...
            pairwise_loss = self.pairwise_loss(intra_sim_map, aux_label)
            loss = main_loss + 0.4 * aux_loss + intra_sim_loss + pairwise_loss
            return loss
...
class PairwiseLoss(nn.Module):
    def __init__(self):
        ...
        self.pw = nn.PairwiseDistance(p=2, eps=1e-6, keepdim=True)

    def forward(self, pred, label):
        b, N, D = pred.size()
        pred = pred.view(-1, D)
        label = label.view(-1, D)
        pw_distance = self.pw(pred, label)
        loss = torch.mean(pw_distance)
