...
__all__ = ['XSNet', 'xsnet16', 'xsnet18', 'xsnet18_v1b', 'xsnet18v2', 'xsnet34',
           'xsnet23',
           'xsnet1x23', 'xsnet1x34',
           'xsnet1x18']
...
class ContextEmbeddingV2(nn.Module):
    def __init__(self, in_channels, norm_layer=nn.BatchNorm2d):
        super(ContextEmbeddingV2, self).__init__()
        self.selayer = SELayer(in_channels, in_channels, 1)

    def forward(self, x):
        attention = self.selayer(x)
        x = x + x * attention
...
class Block(nn.Module):
    ...
    def forward(self, x):
        if self.use_res_connect:
            ...
            return torch.cat([shortcut, self.residual_branch(x)], dim=1)
        else:
...
def xsnet18(pretrained_model=None, **kwargs):
...
def xsnet18v2(pretrained_model=None, **kwargs):
    model = XSNet(Block, [2, 2, 4], [32, 64, 96],
                  context_embedding=ContextEmbeddingV2, **kwargs)
    # model = XSNet(Block, [2, 4, 4], [32, 64, 96], **kwargs)
